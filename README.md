...
| ------- | -------- | ---- | ---------------------- |
| `状态:` | 发布状态 | 蓝色 | 决定文章是否可见       |
| `分类:` | 领域分类 | 绿色 | 文章的核心技术分类     |
| `展示:` | UI 展示  | 橙色 | 置顶、精选、热门等标识 |
| (无)    | 技术标签 | 灰色 | 细粒度的技术关键字     |
...
本博客系统使用多阶段自动化流水线，确保数据一致性和部署效率。

### 1. 标签管理 (Issue Label Manager)

- **文件**: `issue-label-manager.yml`
- **触发条件**: Issue 创建或添加标签。
- **核心功能**:
    - 为新文章自动注入 `状态:草稿` 标签。
    - 确保状态标签互斥（一篇文章只能有一个 `状态:` 标签）。
- **作用**: 在数据同步前规范化 Issue 元数据。

### 2. 数据同步 (并行执行)

这两个任务由标签管理任务完成后触发，或在 Issue 内容/标签变动时直接触发。

#### 文章同步 (Sync Articles)

- **文件**: `sync-articles.yml`
- **核心功能**: 解析 Issue 正文，更新 `public/data/articles.json`。

#### 标签统计 (Sync Tags)

- **文件**: `sync-tags.yml`
- **核心功能**: 重新计算全站分类和标签的使用次数，生成 `public/data/tags.json`。

### 3. 网站部署 (Deploy Site)

- **文件**: `deploy-site.yml`
- **触发条件**: 文章同步或标签统计同步完成后触发。
- **核心功能**: 执行项目构建（Build）并发布到 GitHub Pages。

### 自动化流程图

```mermaid
graph TD
    A[创建/编辑 Issue] --> B[标签管理任务]
    B --> C[文章内容同步]
    B --> D[标签统计同步]
    C --> E[网站构建发布]
    D --> E[网站构建发布]
